...
export const SpeedGame: React.FC<SpeedGameProps> = ({ level, onComplete, onTimeUp, onBackToSelection, onGameStateChange }) => {
  ...
  const [switchSpeedSec, setSwitchSpeedSec] = useState<number>(1.0);
  const gameEndTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const startTimestampRef = useRef<number | null>(null);

  // Switch speed (ms) from slider
  const speed = Math.round(switchSpeedSec * 1000);
  ...
  const startGame = useCallback(() => {
    ...
    // Schedule automatic game end based on selected duration
  ...
  const endGame = useCallback(async () => {
    ...
    if (startTimestampRef.current !== null) {
      const finalSeconds = Math.floor((Date.now() - startTimestampRef.current) / 1000);
  ...
  // Scheduling moved to startGame to avoid rescheduling during gameplay
          ...
          <div className="mt-8 space-y-6 text-left">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Продължителност: <span className="font-semibold">{durationSeconds} сек</span>
              </label>
              <input
                type="range"
                min={30}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Скорост на смяна: <span className="font-semibold">{switchSpeedSec.toFixed(1)} сек</span>
              </label>
              <input
                type="range"
                min={0.5}
                max={1.5}
                step={0.1}
                value={switchSpeedSec}
                onChange={(e) => setSwitchSpeedSec(Number(e.target.value))}
                className="w-full accent-green-600"
              />
              <div className="flex justify-between text-xs text-gray-500 mt-1">
                <span>0.5s</span>
