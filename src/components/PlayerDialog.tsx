...
export function PlayerDialog({ open, onOpenChange, onGameStart }: PlayerDialogProps) {
      ...
      if (storedTeams) {
        try {
          ...
          if (parsedTeams && parsedTeams.length > 0) {
            ...
            // If teams exist, switch to teams tab by default
            setActiveTab("teams");
          }
        } catch (e) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`${activeTab === "teams" ? "sm:max-w-7xl" : "sm:max-w-4xl"} max-h-[95vh] overflow-hidden border-0 p-0 bg-gradient-to-br from-orange-400 via-orange-200 to-orange-200 shadow-2xl`}>
        
        {/* Main Content */}
        <div className="flex-1 p-6 overflow-hidden">
          <Tabs defaultValue="players" className="w-full h-full flex flex-col" onValueChange={setActiveTab}>
            <TabsList className="grid grid-cols-2 mb-6 bg-white/60 backdrop-blur-sm border border-white/30 shadow-sm">
              <TabsTrigger 
                value="players" 
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <Button 
                ...
                className="bg-white/60 backdrop-blur-sm border-white/50 hover:bg-white/70"
              >
                Затвори
              </Button>
              <Button 
